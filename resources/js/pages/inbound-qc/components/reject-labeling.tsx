...
const RejectLabelingModal: React.FC<Props> = React.memo(({ visible, onClose }) => {
    const { form, post, processing } = useAntdInertiaForm('Reject Labelling');
    ...
    const handleOk = useCallback(async () => {
        ...
        post({
            url: '/inbound-qc/reject-labelling',
            data: formValues,
            onSuccess: () => {
                onClose();
                form.resetFields();
    ...
    return (
        <Modal
            title={
                <>
                    <QrcodeOutlined /> Input Reject Labeling
                </>
            }
            open={visible}
            onOk={handleOk}
...
export default RejectLabelingModal;
